...
from jaxopt._src.scipy_wrappers import ScipyWrapper, ScipyMinimizeInfo, LbfgsInvHessProductPyTree
from jaxopt._src.scipy_wrappers import make_onp_to_jnp, jnp_to_onp, pytree_topology_from_example
...
class ScipyMinimize(ScipyWrapper):
    ...
    def optimality_fun(self, sol, *args, **kwargs):
        ...
        if self.automate_jac:
            return self._grad_fun(sol, *args, **kwargs)
        else:
            return self.fun(sol, *args, **kwargs)[1]
    ...
    def _run(self, init_params, bounds, *args, **kwargs):
        ...
        if self.callback is not None:
            def scipy_callback(x_onp: np.ndarray):
        ...
        if self.jac is None: 
            if self.automate_jac:
                def scipy_fun(x_onp: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
                    x_jnp = onp_to_jnp(x_onp)
                    value, grads = self._value_and_grad_fun(x_jnp, *args, **kwargs)
                    return np.asarray(value, self.dtype), jnp_to_onp(grads, self.dtype)
            else:
                def scipy_fun(x_onp: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
                    x_jnp = onp_to_jnp(x_onp)
                    value, grads = self.fun(x_jnp, *args, **kwargs)
                    return np.asarray(value, self.dtype), jnp_to_onp(grads, self.dtype)
            ...
            def scipy_fun(x_onp: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
                x_jnp = onp_to_jnp(x_onp)
                value = self.fun(x_jnp, *args)
                return np.asarray(value, self.dtype) 
            
            def jac_fun(x_onp: np.ndarray) -> np.ndarray:
                x_jnp = onp_to_jnp(x_onp)
                jac_value = self.jac(x_jnp, *args)
                return np.asarray(jac_value, self.dtype) 
            
            def hessp_func(x_onp: np.ndarray, p_onp: np.ndarray) -> np.ndarray:
                x_jnp = onp_to_jnp(x_onp)
                p_jnp = onp_to_jnp(p_onp)
                hessp_value = self.hessp(x_jnp, p_jnp, *args)
                return np.asarray(hessp_value, self.dtype) 
        ...
        if hasattr(res, 'hess_inv'):
            if isinstance(res.hess_inv, osp.optimize.LbfgsInvHessProduct):
                hess_inv = LbfgsInvHessProductPyTree(res.hess_inv.sk,
                                                    res.hess_inv.yk)
            elif isinstance(res.hess_inv, np.ndarray):
        ...
        if self.automate_jac:
            ...
            if self.jit:
                ...
                if self.jac is None:
                    ...
                    self.jac = jax.jit(self.jac)
                    self.hessp = jax.jit(self.hessp)
...
class ScipyBoundedMinimize(ScipyMinimize):
    ...
    def _fixed_point_fun(self, sol, bounds, args, kwargs):
        if self.automate_jac:
            ...
            step = tree_sub(sol, self.fun(sol, *args, **kwargs)[1]) 
